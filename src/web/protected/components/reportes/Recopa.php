...
//            echo "el excel esta bien, el peo debe ser entonces el tiempo que tarda para generar el reporte";
        ...
        public static function defineTD($date,$dateNow,$SOA_date_top,$amount)
        {
            if($SOA_date_top >= $dateNow && $SOA_date_top <= Reportes::define_due_date("6", $dateNow,"+")){
                return  self::defineColorTD($amount,null) .">".Yii::app()->format->format_decimal($amount);
            }
            elseif($SOA_date_top < Reportes::define_due_date("7", $date,"-")){
