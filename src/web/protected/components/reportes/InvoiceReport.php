...
class InvoiceReport extends Reportes
{
     ...
     */
    public static function reporte($fromDate,$toDate,$typeReport,$paymentTerm=null,$dividedInvoice)
    {
        //Estilos
        ...
        $documents=self::getModel($fromDate, $toDate,$typeReport,$paymentTerm,$dividedInvoice, NULL);
    ...
    <td colspan='12'><h1>{$typeReport}</h1>".str_replace("-","",$fromDate)." - ".str_replace("-","",$toDate)." al ".str_replace("-","",date('Y-m-d'))."</td>    
        ...
        $totalSummary=self::getModel("2014-01-27", "2014-02-02",$typeReport,$paymentTerm,$dividedInvoice, TRUE);
            $body.="<tr>
         ...
         $body.="</table><br>";
         $body.="<table style='width: 100%;'>";
        foreach($totalSummary as $key => $totals)
        {
            $body.="<tr>
                       <td colspan='12'>SUMMARY {$typeReport}</td>
                    </tr>
                    <tr>
                       <td colspan='2'>PERIODO </td>
                       <td {$styleDescription} colspan='10'> ".Utility::formatDateSINE($fromDate,"F j")." - ".Utility::formatDateSINE($toDate,"F j")."</td>
                    </tr>
                    <tr>
                       <td $style_provisiones colspan='2'><b>MINUTOS</b></td>
                       <td $style_totals colspan='2'>".Yii::app()->format->format_decimal($totals->minutes,3)."</td>
                       <td $style_sori ><b>MINUTOS</b></td>
                       <td $style_totals colspan='2'>".Yii::app()->format->format_decimal($totals->fac_minutes,3)."</td>
                       <td $style_diference ><b>MINUTOS</b></td>
                       <td $style_totals colspan='3'>".Yii::app()->format->format_decimal($totals->min_diference,3)."</td>
                    </tr>
                    <tr>
                       <td $style_provisiones colspan='2'><b>MONTO $</b></td>
                       <td $style_totals colspan='2'>".Yii::app()->format->format_decimal($totals->amount,3)."</td>
                       <td $style_sori ><b>MONTO $</b></td>
                       <td $style_totals colspan='2'>".Yii::app()->format->format_decimal($totals->fac_amount,3)."</td>
                       <td $style_diference ><b>MONTO $</b></td>
                       <td $style_totals colspan='3'>".Yii::app()->format->format_decimal($totals->monto_diference,3)."</td>
                   </tr>";
        }
        $body.="</table>";
     ...
     * @return type
     */
    private static function getModel($startDate, $endDate, $typeReport, $paymentTerm=null, $dividedInvoice, $colls) 
    {
        $from=">=";
        $to="<=";
        $divided="";
        ...
        if($colls==NULL){
            $colls="b.*, (b.fac_minutes-b.minutes) AS min_diference, (b.fac_amount-b.amount) AS monto_diference";
        }else{
            $colls="SUM(b.minutes) AS minutes,
