        ...
        $("#excel").hover(function()
        {
            switch ($(this).attr("id"))
            {
                case "excel":
                    ...
                    if (valid_input == 1)
                        $(".excel_a").attr("href", "Site/Excel?" + $('#formulario').serialize() + "");
                    break;
            }
        });
    }
    ...
    function elijeOpciones(obj)
    {
        ...
        switch (nombre) {
            ...
            case "reteco":
                ...
                $(".termino_pago").removeClass("termino_pago_sum_re termino_pago_refac_reprov");
                break;
            case "refac":
            ...
            case "refi_prov":
                ...
                $(".termino_pago_refac_reprov").removeClass("termino_pago_sum_re termino_pago");
                break;
            case "waiver":
...
//                var mostrar = [".fecha,.intercompany,.no_activity,.matches,.termino_pago,.termino_pago_sum_re,.type_termino_pago,.type_termino_pago_sum_re,.termino_pago_refac_reprov,.note,#id_termino_pago option[value='todos']"];
    ...
    function export_report(click)
    {
        ...
        {
            $SINE.UI.msjCargando("", "");
            $SINE.UI.msjChange("<h2>Faltan campos por llenar </h2>", "stop.png", 500, "60px");
        } else {
            var id = $(click).attr('id');
            if (id == "mail") {
                $SINE.UI.msjCargando("<h2>Enviando Email</h2>", "cargando.gif");
                $SINE.AJAX.send("GET", "/site/CalcTimeReport", $("#formulario").serialize(), "<h2>Enviando Email</h2>");
                $SINE.AJAX.send("POST", "/site/mail", $("#formulario").serialize(), null);
            }
            else if (id == "previa") {
                $SINE.UI.msjCargando("<h2>Cargando Vista Previa</h2>", "cargando.gif");
                $SINE.AJAX.send("GET", "/site/CalcTimeReport", $("#formulario").serialize(), "<h2>Cargando Vista Previa</h2>");
...
//                 $SINE.UI.genExcel("/site/Excel",$("#formulario").serialize()); 
                ...
                $(document).ajaxError(function() {
                    ...
                    $SINE.AJAX.send("GET", "/site/CalcTimeReport", $("#formulario").serialize(), "<h2>Exportando Archivo Excel </h2>");
                    $SINE.AJAX.send("GET", "/site/Excel", $("#formulario").serialize(), null);
                });
            }
        }
    ...
    function genProvisions(click)
    {
        ...
        {
            $SINE.UI.msjCargando("", "");
            $SINE.UI.msjChange("<h2>Debe llenar al menos el periodo </h2>", "stop.png", 1000, "60px");
        } else {
...
//                console.log(campos[i]);
    ...
    function emergingView(body, additional)
    {
        ...
        $("body").append(msj);
        msj.slideDown('slow');
        $(".emergingBackground").click(function() {
            $(".emergingBackground,.emergingView").slideToggle("slow");
        });
    }
    /**
